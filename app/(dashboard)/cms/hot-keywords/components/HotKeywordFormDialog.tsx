...
// Server Actions
import { createHotKeyword, updateHotKeyword, type ActionState } from '../actions/hotKeyword';
...
export default function HotKeywordFormDialog({
    initialData,
    trigger,
    title,
    open: controlledOpen,
    ...
    const dialogTitle = title || (isEdit ? '編輯關鍵字' : '新增關鍵字');

    const form = useForm({
        resolver: zodResolver(HotKeywordSchema),
        defaultValues: {
            title: '',
            linkUrl: '',
            order: 0,
    ...
    const onSubmit = (values: z.infer<typeof HotKeywordSchema>) => {
        startTransition(async () => {
            try {
                ...
                if (isEdit && initialData?.id) {
                    ...
                    result = await updateHotKeyword(initialData.id, formData);
                } else {
                    result = await createHotKeyword(null, formData);
                }
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            >
                ...
                <DialogHeader className="p-8 pb-4 bg-white border-b border-slate-100">
                    ...
                    <DialogDescription className="text-slate-500 font-medium tracking-tight">請填寫關鍵字資訊，完成後點擊儲存變更。</DialogDescription>
                </DialogHeader>
                
                <ScrollArea className="flex-1 px-8 py-6">
                    <Form {...form}>
                        <form id="hot-keyword-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <MuiFormItem label="標題" required>
                                                <Input {...field} className="border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-50 transition-all bg-slate-50/30 font-medium" placeholder="例：日本賞櫻" disabled={isPending} />
                                            </MuiFormItem>
                                        )}
                                    />
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="flex flex-row items-center justify-between rounded-xl border border-slate-200 p-4 bg-white hover:shadow-sm transition-all">
                                                <div className="space-y-0.5">
                                                    ...
                                                    <FormDescription className="text-xs text-slate-500">關閉後前台將不會顯示此關鍵字</FormDescription>
                                                </div>
                                                <FormControl>
                                                    <Switch
                                                        checked={field.value}
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-6 rounded-2xl border border-slate-100 bg-slate-50/30 space-y-5">
                                        <FormField
                                            ...
                                            name="gaEvent"
                                            render={({ field }) => (
                                                <MuiFormItem label="事件類型" description="建議使用 ga-click">
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <MuiFormItem label="事件名稱">
                                                    <Input {...field} value={field.value || ''} className="bg-white border-slate-200 focus:ring-4 focus:ring-blue-50" placeholder="hot_keyword_click" disabled={isPending} />
                                                </MuiFormItem>
                                            )}
                                        />
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <MuiFormItem label="事件類別">
                                                    <Input {...field} value={field.value || ''} className="bg-white border-slate-200 focus:ring-4 focus:ring-blue-50" placeholder="HotKeyword" disabled={isPending} />
                                                </MuiFormItem>
                                            )}
                                        />
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <MuiFormItem label="事件標籤">
                                                    <Input {...field} value={field.value || ''} className="bg-white border-slate-200 focus:ring-4 focus:ring-blue-50" placeholder="Search Japan" disabled={isPending} />
                                                </MuiFormItem>
                                            )}
                                        />
                                    </div>
                ...
                <DialogFooter className="p-6 bg-slate-50 border-t border-slate-100 gap-3">
                    ...
                    >
                        {isPending ? '處理中...' : (isEdit ? '儲存變更' : '建立關鍵字')}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
