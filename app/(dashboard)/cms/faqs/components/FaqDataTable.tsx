...
// --- 1. 定義可拖曳的 Row 元件 ---
const DraggableRow = ({ row }: { row: Row<Faq> }) => {
    const {
        attributes,
        listeners,
        setNodeRef,
...
interface FaqActionsCellProps {
    faq: Faq;
    onDeleteClick: (id: string) => void;
}

const FaqActionsCell = ({ faq, onDeleteClick }: FaqActionsCellProps) => {
    ...
    const columns = React.useMemo<ColumnDef<Faq>[]>(
        () => [
            ...
            },
            {
                ...
                header: '回答預覽',
                cell: ({ row }) => {
                    const htmlContent = row.getValue('answer') as string;
                    const textContent = htmlContent?.replace(/<[^>]+>/g, '') || '';
                    
                    return (
                        <div className="text-sm text-muted-foreground line-clamp-2 max-w-[400px]" title={textContent}>
                            {textContent}
                        </div>
                    );
                },
                        ...
                        {table
                            ...
                            .map((column) => {
                                return (
                                    ...
                                    >
                                        {column.id === 'question' ? '問題' : column.id === 'answer' ? '回答' : column.id}
                                    </DropdownMenuCheckboxItem>
                                );
                            })}
                    </DropdownMenuContent>
